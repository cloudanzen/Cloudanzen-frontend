...
function VeracodeConnectModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await veracodeService.connect({
        apiId: apiId.trim(),
        apiKey: apiKey.trim(),
      ...
      setError(
        (err as { message?: string })?.message ??
          t('cards.veracode.connectFailed'),
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      ...
      >
        <h2 className="text-lg font-semibold mb-1">
          {t('cards.veracode.connect')}
        </h2>
        <p className="text-sm text-gray-500 mb-4">
          {t('cards.veracode.connectDescription')}
        </p>
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('cards.veracode.apiId')}
            </label>
            <input
              type="text"
              value={apiId}
              onChange={(e) => setApiId(e.target.value)}
              placeholder="Veracode API ID"
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Production Veracode"
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900"
            />
          </div>
        </div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <Button
            type="submit"
            disabled={loading}
            className="bg-[#E87722] hover:bg-[#c96314] text-white"
          >
            {loading
...
export function VeracodeCard({
  ...
  async function handleScan(id: string) {
    setScanningId(id);
    try {
      await veracodeService.runScan(id);
      onToast('success', t('cards.veracode.scanStarted'));
    } catch {
      onToast('error', t('cards.veracode.scanStartFailed'));
    } finally {
      setScanningId(null);
    }
  }
  ...
  async function handleDisconnect(id: string, label: string | null) {
    const confirmed = await confirm({
      title: t('cards.veracode.disconnectTitle'),
      description: t('cards.veracode.disconnectDescription', {
        target: label ?? id,
      }),
    ...
    try {
      await veracodeService.disconnect(id);
      onAccountRemoved(id);
      onToast('success', t('cards.veracode.disconnected'));
    } catch {
      onToast('error', t('cards.veracode.disconnectFailed'));
    } finally {
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <Badge variant={isConnected ? 'default' : 'outline'}>
            {loadingStatus
              ...
              : isConnected
                ? t('cards.veracode.accountsConnected', {
            ...
            >
              <div>
                ...
                <p className="text-xs text-gray-400 font-mono">
                  {t('cards.veracode.findings', {
                    count: account.findingCount,
                  })}
                  {account.lastSyncAt &&
                    ` · ${t('cards.veracode.lastSync', { date: new Date(account.lastSyncAt).toLocaleString() })}`}
                </p>
              </div>
        ...
        <div className="flex flex-wrap gap-2">
          {!loadingStatus && (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-[#E87722] hover:bg-[#c96314] text-white text-sm font-medium"
            >
              {isConnected
                ? t('cards.veracode.addAccount')
                : t('cards.veracode.connect')}
            </button>
          )}
        </div>
      ...
      {showConnectModal && (
        <VeracodeConnectModal
          ...
          onConnected={(account) => {
            ...
            onToast('success', t('cards.veracode.connected'));
