...
function NotionConnectModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!token.trim()) {
      setError(t('cards.notion.requiredFields'));
      return;
    }
    setLoading(true);
    setError('');
    try {
      await notionService.connect(token.trim());
      const status = await notionService.getStatus();
      if (!status.connected || !status.data) {
        throw new Error(t('cards.notion.statusLoadFailed'));
      }
      onConnected(status.data);
    } catch (err: unknown) {
      setError(
        (err as { message?: string })?.message ??
          t('cards.notion.connectFailed'),
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      ...
      >
        ...
        <p className="text-sm text-gray-500 mb-4">
          {t('cards.notion.connectDescription')}
        </p>
        <ol className="text-xs text-gray-600 list-decimal pl-5 mb-4 space-y-1">
          <li>{t('cards.notion.step1')}</li>
          <li>{t('cards.notion.step2')}</li>
          <li>{t('cards.notion.step3')}</li>
        </ol>
        <a
        ...
        >
          {t('cards.notion.openNotion')}
        </a>
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('cards.notion.tokenLabel')}
            </label>
            <input
              type="password"
              value={token}
              onChange={(e) => setToken(e.target.value)}
              placeholder={t('cards.notion.tokenPlaceholder')}
      ...
      {showConnectModal && (
        <NotionConnectModal
          ...
          onConnected={(status) => {
            ...
            onToast('success', t('cards.notion.connected'));
          }}
        />
      )}
    </>
